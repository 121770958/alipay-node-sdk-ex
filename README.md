...
> 支付宝新版App支付nodejs版sdk,扩展了支付宝身份校验，人脸认证.
 ...
 */
var ali = new Alipay({
    appId: '2016080300159077', notifyUrl: 'http://www.xxx.com/callback/alipay',
...
/**
 *
 * 支付宝支付【会员api】通用方法(身份校验/人脸认证)为例子
 * https://opendocs.alipay.com/apis/api_2/alipay.user.certify.open.query
 * @param {String}
 * @param {Object} opts  //按照支付宝官方文档传入业务请求参数（公共参数不用）
 */

ali.commonExectute("alipay.user.certify.open.query", {
    certify_id: "certify_id",
}).then(res => {
    console.log(res.body)   //req.body 支付宝响应体
})
 ...
 * 获取人脸认证初始化签名
 * * @param {Object} opts
 * @param {String} [opts.outTradeId] 订单号
 * @param {String} [opts.cert_name]  身份证名字
 * @param {String} [opts.cert_no]    身份证号码
 * @param {String} [opts.return_url] 返回url
 * @param {String} [opts.identity_type] 必须传入 "CERT_INFO"
 * @param {String} [opts.cert_type] 必须传入 "IDENTITY_CARD"
 * @param {String} [opts.biz_code]
 */
alipay.certifyInitialize({
    outTradeId: "outTradeId",
    cert_name: "cert_name",
    cert_no: "cert_no",
 ...
 * 查询认证结果
 * * @param {Object} opts
 * @param {String} [opts.certify_id] 初始化时候返回的 certify_id
 */
alipay.certifyQuery({
    certify_id: certify_id
}).then((response) => {
    console.log("query response", JSON.parse(res.body))
})
 ...
 * @param {String} [opts.certify_id] 初始化时候返回的 certify_id
 */
const certify_url = alipay.certifyQuery({certify_id: certify_id})
 ...
 * @param {String} [opts.certify_id] 初始化时候返回的 certify_id
 */
const certify_url = alipay.getNativeCertifyUrl({certify_id: certify_id})
