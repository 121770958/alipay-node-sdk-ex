 ...
 */
props.certifyInitialize = function (opts) {

    var biz_content = {
        outer_order_no: opts.outTradeId,
        biz_code: opts.biz_code,
        identity_param: {
            identity_type: opts.identity_type,
            cert_type: opts.cert_type,
            cert_name: opts.cert_name,
            cert_no: opts.cert_no
        },
        merchant_config: {return_url: opts.return_url}
    };
    var params = this.makeParams('alipay.user.certify.open.initialize', biz_content);
    if(this.appAuthToken) {
        params.app_auth_token = this.appAuthToken;
    }
 ...
 * @returns {Promise<Object>}
 */
props.certify = function (opts) {
    var biz_content = {
        certify_id: opts.certify_id
    };
    var params = this.makeParams('alipay.user.certify.open.certify', biz_content);
    if(this.appAuthToken) {
        params.app_auth_token = this.appAuthToken;
    }
 ...
 * @returns string
 */
props.getCertifyUrl = function (opts) {
    var biz_content = {
        certify_id: opts.certify_id
    };
    var params = this.makeParams('alipay.user.certify.open.certify', biz_content);
 ...
 * @returns string
 */
props.getNativeCertifyUrl = function (opts) {
    var biz_content = {
        certify_id: opts.certify_id
    };
    var params = this.makeParams('alipay.user.certify.open.certify', biz_content);
    ...
    params.alipay_sdk =  "alipay-sdk-java-4.39.79.ALL"; // "alipay-sdk-java-dynamicVersionNo";
 ...
 * @returns {Promise<Object>}
 */
props.certifyQuery = function (opts) {
    var biz_content = {
        certify_id: opts.certify_id,
    };
    var params = this.makeParams('alipay.user.certify.open.query', biz_content);
    if(this.appAuthToken) {
        params.app_auth_token = this.appAuthToken;
    }
...
/**
 * 支付宝支付会员api通用方法(身份校验)   https://opendocs.alipay.com/apis/api_2/alipay.user.certify.open.query
 * @param apiStr    传入例子: "alipay.user.certify.open.initialize"
 * @param opts   传入例子: {outer_order_no:"outer_order_no"} 所有业务相关参数（严格按照支付宝官方文档传入属性名字）如下是文档链接
 * https://opendocs.alipay.com/apis/api_2/alipay.user.certify.open.initialize
 * @returns {Promise<Object>}
 */
props.commonExectute = function (apiStr,opts) {
    var biz_content = {};

    Object.keys(opts).forEach((key)=>{
        biz_content[key] = opts[key]
    })

    var params = this.makeParams(apiStr, biz_content);
    if(this.appAuthToken) {
        params.app_auth_token = this.appAuthToken;
    }
